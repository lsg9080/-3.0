<template>
  <div :class="['recommend', flag ? 'bigSize' : '']">
    ...
    <split></split>
    <div v-if="isShowArticle" class="articleNavBar">
      <div class="title"></div>
...
// import { homePage, wechatAppId } from "../common/env";
import {
  encryption,
  encryptionRechange,
  formatDate,
...
export default {
  ...
  data() {
    return {
      ...
      totalRecord: 0, //总条数
      nextPageIds: [], //文章已加载id
      isShowArticle: true // 是否启用【营养必看】模块
    };
  },
  computed: {
    //主模块
  ...
  created() {
    var $this = this;
    // storage.setItem("YKOpenId", "O8JnqtxRYwIOuoaA1LEik9zGK_k");
    storage.setItem("YKOpenId", 'ocWCVwjK_kseL-qYB83YuQYk6l3Q')

    var getopenid = storage.getItem("YKOpenId");
    if (getopenid == null || getopenid == undefined || getopenid == "") {
      ...
      console.log( this.$wechatAppId);
      console.log("queryParams:" + queryParams);
      console.log("opencode:" + opencode);
      if (opencode == null || opencode == undefined || opencode == "") {
        window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${ this.$wechatAppId}&redirect_uri=${this.$homePage}&response_type=code&scope=snsapi_base&state=123#wechat_redirect`;
      } else {
        this.getopenid()
      }
    }
    ...
    this.GetModuleList();
    this.GetIllness();
    this.openid = storage.getItem("YKOpenId");
  ...
  methods: {
    ...
    /**
     * 001，首页标题
     * 000，不显示营养必看文章（1：不显示，0：显示[默认]
     * 004（1：启用，0：不启用），N选M，如果是1选1，那个菜默认选中
     * 005（0：不启用，1：启用），病人餐早午晚三餐必须全选才能下单
     */
    GetSetting() {
      if (!storage.getItem("userStatus")) return;
      var _this = this;
      ...
      _this.$ajax
        ...
        .then(resp => {
          if (resp.data.code == 0) {
            let setting = resp.data.data;
            this.$store.commit("settingSwitch", setting);
            // let json = [
            //   { code: "000",  value: "1" }
            //         { code: "001", name: "haha", value: "1" },
            //         { code: "004", name: "N选M，如果是1选1，那个菜默认选中", value: "1" },
            //         { code: "005", name: "病人餐早午晚三餐必须全选才能下单", value: "1" }
            //       ];
            //       setting = [...setting,...json]
            //余额预警值
            let findVal = setting.find(item => {
              return item.code === "003";
            });
            _this.warningValue = findVal.value;
            //启用充值
            let findVal2 = setting.find(item => {
              return item.code === "002";
            });
            _this.enableRecharge = findVal2.value;
            storage.setItem("enableRecharge", _this.enableRecharge);
            // 页面标题
            let title = setting.find(item => {
              return item.code === "001" && item.value != "";
            });
            if (title) {
              document.title = title.value;
            }
            // 是否开启 【营养必看】 模块
            let article = setting.find(item => {
              return item.code === "000" && item.value == 1;
            });
            if (article) {
