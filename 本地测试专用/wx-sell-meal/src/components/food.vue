    ...
    <div v-show="showFlag" class="foodDesc">
      <cube-scroll ref="scrollFood">
        <div class="food-content">
          <div class="image-header">
            <div class="slideImgBox" v-if="foodInfo.picUrlList && foodInfo.picUrlList.length > 1">
              <cube-slide ref="bannerslide">
                <cube-slide-item v-for="(item, index) in foodInfo.picUrlList" :key="index">
            ...
            <div v-else class="img" :style="{backgroundImage: 'url(' + foodInfo.picUrlList + ')'}"></div>
            <div class="back" @click="hide">
              <i class="icondelete"></i>
            </div>
            <div class="imgContent">
              <h1 class="title">{{ foodInfo.menuName }}</h1>
              <div v-if="detailFood.orderingMode == 1" class="cartcontrolContent">
                <cartcontrol @add="addFood" :food="detailFood"></cartcontrol>
              </div>
          ...
          <div v-if="foodInfo.menuDescription" class="desc" :class="[toggleStatus ? 'on' : '']">
            {{ formatDescText }}
            <span
              @click="toggleDesc"
              v-if="needMore"
          ...
          <div class="content">
            ...
            <div class="rawMaterial" :class="[showAll2 ? 'on' : '']">
              <div
                v-show="foodInfo.foodList && foodInfo.foodList.length>2"
            ...
            <div class="title title3">
              三大能量来源比
              <span @click="moreMeaning" class="moreMeaning">了解它的意义</span>
            </div>
            <div class="EnergyRatio">
              <ul>
                <li
                  v-if="foodInfo.energyOfSourcePercentList"
                  v-for="(item, index) in foodInfo.energyOfSourcePercentList"
                  :key="index"
            ...
            <div class="nutritioPart">
              ...
              <div
                v-if="foodInfo.nutrientOtherList && foodInfo.nutrientOtherList.length > 5"
...
export default {
  ...
  computed: {
    splitClass() {
      if (
        this.foodInfo.foodList &&
        this.foodInfo.foodList.length == 1 &&
        this.showAll2 == false
      ) {
        return "split1N";
      }
    ...
    formatDescText() {
      if (this.toggleStatus || !this.needMore) {
        ...
        return (
          this.foodInfo.menuDescription.substring(0, this.maxLength) + "..."
        );
      }
    },
    word() {
  ...
  methods: {
    ...
    detailSelFood() {
      if (this.detailFood.orderingMode == 2) {
        ...
        // N选M
        this.$parent.choosedMeal(
          this.NMItemPrams.outIndex,
          this.NMItemPrams.mealItem,
          this.NMItemPrams.price,
          this.NMItemPrams.itemN,
          this.NMItemPrams.itemM,
          this.NMItemPrams.itemMenuTypeName
        );
      }
    },
    moreMeaning() {
...
.foodDesc {
  ...
  .content {
    ...
    .title {
      ...
      overflow: hidden;
      &.title1 {
        background: url(../assets/meal/food1@2x.png) no-repeat left center;
        background-size: 18px 18px;
      }
      &.title2 {
        background: url(../assets/meal/food2@2x.png) no-repeat left center;
        background-size: 18px 18px;
      }
      &.title3 {
        background: url(../assets/meal/food3@2x.png) no-repeat left center;
        background-size: 18px 18px;
      }
      .moreMeaning {
        ...
        font-size: 14px;
        font-weight: 600;
        background: url(../assets/meal/icon04@2x.png) left center no-repeat;
    ...
    .mainNutritioPart {
      ul {
        ...
        li {
          ...
          &:nth-child(1) {
            background-image: linear-gradient(45deg, #7cdbb2 0%, #21c67f 100%);
            box-shadow: 0 2px 10px 0 rgba(33, 198, 127, 0.25);
            border-radius: 4px;
          }
          &:nth-child(2) {
            background-image: linear-gradient(45deg, #ffd678 0%, #fdbf32 100%);
            box-shadow: 0 2px 10px 0 rgba(253, 191, 50, 0.25);
            border-radius: 4px;
          }
          &:nth-child(3) {
            background-image: linear-gradient(45deg, #ffaa85 0%, #ff8361 100%);
            box-shadow: 0 2px 10px 0 rgba(255, 131, 97, 0.25);
            border-radius: 4px;
          }
          &:nth-child(4) {
            background-image: linear-gradient(45deg, #aad5ff 0%, #72acff 100%);
            box-shadow: 0 2px 10px 0 rgba(115, 172, 255, 0.25);
            border-radius: 4px;
          }
          &:nth-child(5) {
          }
          background-image: linear-gradient(45deg, #adacfb 0%, #7574f5 100%);
