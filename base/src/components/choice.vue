...
import { authPageBaseUri, homePage, wechatAppId } from "../common/js/config";
import {
  encryption,
  formatDate,
  formatReq,
...
export default {
  ...
  methods: {
    ...
    redirectToAuthPageUrl() {
      ...
      let url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${this.config.appid}&redirect_uri=${homePage}&response_type=${this.config.responseType}&scope=${this.config.scope}&state=${this.config.state}&connect_redirect=1#wechat_redirect`;
