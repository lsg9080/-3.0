        ...
        <div class="shopcart">
            <div class="content" @click="toggleList">
                ...
                <div class="content-right">
                    <div :class="analysisClass" class="analysis" @click.stop.prevent="analysis">
                        营养分析
                    </div>
                    <div :class="payClass" class="pay" @click.stop.prevent="pay">
                        去下单
                    </div>
                </div>
            </div>
                        ...
                        <cube-scroll ref="scrollCar" :data="selectFoods">
                            <ul v-for="(item,index) in SingleSelectFoods" :key="index">
								...
								<template v-for="repeatItem in item.repeatList">
									<div class="carMealTimes">
										{{repeatItem.repeatName.replace(/\"/g,'')}}
									</div>
									<li v-for="(food, foodIndex) in repeatItem.food" :key="food.code+foodIndex" class="food">
                                        ...
                                        <div class="cartcontrol-wrapper">
                                            <div class="cartcontrol">
                                                <div class="cart-decrease">
                                                    <span @click="removeFood(repeatItem.food)"
                                                        class="inner iconjianshao"
                                                    ></span>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <ul v-for="item in NMSelectFoods">
                                <div class="carTag">{{item.moduleName.replace(/\"/g,'')}}</div>
                                <template v-for="repeatItem in item.repeatList">
                                    <div class="carMealTimes NSelectMFoodTimes">
                                        {{repeatItem.repeatName.replace(/\"/g,'')}}
                                    </div>
                                    <template v-for="NMItem in repeatItem.NMList">
                                        <div class="carMealTimes carMealTimes2">
                                            {{NMItem.nmName.replace(/\"/g,'')}}（{{NMItem.menuList[0].N}}选{{NMItem.menuList[0].M}}）
                                        </div>
        ...
        <div class="progress-wrapper" :class="progressClass">
            <div class="progress">
                ...
                <div class="ball" :style="{ left: ballLeft + '%' }">
                    <div class="counter" :class="counterClass">{{totalProgress}}<span>kcal</span></div>
                </div>
            </div>
        </div>
        <!-- 营养素摄入量进度条 end -->
...
import { wechatAppId } from '../common/js/config'
...
export default {
    ...
    computed: {
        ...
        totalPrice() {
            ...
            for(let orderingMode in orderingModeGroup){
				 if(orderingMode==3){
					...
					for(let index in moduleIdGroup){//遍历多个n选M
		...
		//进度条小球位置
		ballLeft(){
			var left = (Math.round(this.totalProgress / 4000 * 10000) / 100.00)
			if(left > 100){
				return 99
			}
			return left
    ...
    methods: {
        ...
        removeFood(food){
            food.count=0
            if(food.orderingMode==2)//套餐
                this.$emit('shopCarDecreaseFood', food)
            else
                this.$emit('shopCarDecreaseNMFood', food)//N选M
            // debugger
		...
		jsSDK2(params,isShowFenXi) {
			...
			if (typeof window.WeixinJSBridge === 'undefined') {
				if (document.addEventListener) {
					document.addEventListener('WeixinJSBridgeReady', function () { _this.onBridgeReady2(params) }, false)
				} else if (document.attachEvent) {
					document.attachEvent('WeixinJSBridgeReady', function () { _this.onBridgeReady2(params) })
					document.attachEvent('onWeixinJSBridgeReady', function () { _this.onBridgeReady2(params) })
				}
			} else {
            ...
            _this.$ajax
                ...
                .then(res => {
                    if (res.data.code == '0') {
                        ...
                        // enableRecharge 启用充值
                        // debugger
						let chaPrice = (parseFloat(_this.totalPrice) - parseFloat(_this.paymentList[0].balance)).toFixed(1)
						let dialogCon = `当前最新余额￥${parseFloat(_this.paymentList[0].balance).toFixed(1)}<br/>还差￥${chaPrice}`
        ...
        .content-left {
            ...
            .logo-wrapper {
                ...
                .logo {
                    width: 29px;
                    height: 28px;
                    background: url(../assets/car1@2x.png) center center
                        no-repeat;
                    background-size: 100% 100%;
                    &.highlight {
                        background: url(../assets/car2@2x.png) center center
                            no-repeat;
                        background-size: 100% 100%;
                    }
                }
    ...
    .shopcart-list {
        ...
        .list-header {
            ...
            .empty {
                ...
                font-size: 14px;
                color: #505050;
                background: url(../assets/meal/empty@2x.png) left center
                    no-repeat;
    ...
    .progress {
        ...
        .ball {
            ...
            .counter {
                ...
                font-size: 14px;
                color: #ffc109;
                background: url(../assets/meal/progress1@2x.png) center center
                    no-repeat;
                background-size: 100% 100%;
                min-width: 68px;
				height: 34px;
                ...
                &.counter2 {
                    color: #6fc8a2;
                    background: url(../assets/meal/progress2@2x.png) center
                        center no-repeat;
                    background-size: 100% 100%;
                }
                &.counter3 {
                    color: #ff8361;
                    background: url(../assets/meal/progress3@2x.png) center
                        center no-repeat;
                    background-size: 100% 100%;
				}
				&.counterDefault{
