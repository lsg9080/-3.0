...
<template>
  <div class="rate" :class="{ disabled: disabled }">
    <div class="StarsWrap">
      ...
      >
        <i v-if="xing <= 2" :class="[xing > index ? 'on1' : '', 'iconstar']" />
        <i v-else-if="xing == 3" :class="[xing > index ? 'on2' : '', 'iconstar']" />
        <i v-else-if="xing > 3" :class="[xing > index ? 'on3' : '', 'iconstar']" />
      </div>
    </div>
    <div v-if="xing <= 2" class="comment">{{ rateScoreText }}</div>
    <div v-else-if="xing == 3" class="comment on1">{{ rateScoreText }}</div>
    <div v-else-if="xing > 3" class="comment on2">{{ rateScoreText }}</div>
  </div>
</template>
