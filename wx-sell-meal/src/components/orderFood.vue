<template>
  <div class="goods">
    <!-- <meal-mode
        ...
        ></meal-mode> -->
    <div class="mealType">
      <div class="tab-item" :class="{ on: index === nowIndex }" @click="toggleTabs(index, item.id, item.name)" v-for="(item, index) in moduleList"
        :key="item.id">
        {{ item.name }}
      </div>
    </div>
    ...
    <div class="mealTimesItem">
      <cube-scroll ref="mealScroll" :data="mealTimesList" direction="horizontal" class="horizontal-scroll-list-wrap">
        <ul class="list-wrapper">
          <li v-if="currentRepastList" v-for="(item, index) in currentRepastList" :key="index" class="list-item" :class="{ active: index === currentIndex }"
    ...
    <div class="tab_content">
      <!-- 1：单点，2：套餐，3：N选M -->
      <!-- 单点模式 -->
      <template v-if="currentMenuList.orderingMode === 1" && dandian>
        <div class="goodsMeal">
          <div class="menu-wrapper" ref="menuWrapper">
            <ul>
              <li v-for="(item,innerIndex) in currentMenuList.dataList[0].repastDataList" class="menu-item" :key="innerIndex"
                :class="{current:currentMealIndex === innerIndex}" @click="selectMenu(innerIndex,$event)" ref="menuList">
          ...
          <div class="foods-wrapper" ref="foodsWrapper">
            <ul ref="ulfoodsWrapper">
              <li v-for="item in currentMenuList
									.dataList[0].repastDataList" class="food-list" ref="foodList">
                <h1 class="title">
                  {{ item.menuTypeName }}
                </h1>
          ...
          <div v-show="currentIndex == index">
            <div class="foodScroll">
              <cube-scroll ref="scroll" :data="currentMenuList.dataList[0].menuList">
                <ul class="setMeal">
                  <li v-for="(item, index) in currentMenuList
											.dataList[0].menuList" :key="index" class="setMealItem">
                    ...
                    <template v-if="
                      ...
                      <div class="mealMoreImg">
                        <h2 class="name" @click="selectFood(item, $event)">
                          {{ item.name }}
                        </h2>
                        <div class="imgArrScroll">
                          <cube-scroll ref="scroll" :data="item.picArr" direction="horizontal" nest-mode="free" class="img-scroll-list-wrap">
                            <ul class="list-wrapper">
                              <li v-for="(itemImg,
																index) in item.picUrlList" :key="index" class="list-item">
                                <img v-lazy="itemImg" />
                              </li>
                            </ul>
                          </cube-scroll>
          ...
          <div v-show="currentIndex == index">
            <div class="foodScroll">
              <cube-scroll ref="scroll" :data="
								...
								">
                <div v-for="(item, outIndex) in currentMenuList
										.dataList[0].repastDataList" :key="outIndex"
                  ...
                  <ul class="setMeal NSelectM">
                    <li v-for="(mealItem,
											index) in item.menuList" :key="index" class="setMealItem" @click="selectFood(mealItem, $event, outIndex, mealItem,currentMenuList.dataList[0].price, item.N, item.M, item.menuTypeName)">
                      <div class="selBTn" @click.stop.prevent="choosedMeal(outIndex,mealItem,currentMenuList.dataList[0].price,item.N,item.M,item.menuTypeName)">
                        <i :class="[
														'iconselectdefault2',
														mealItem.count > 0
															? 'iconxuanze'
    ...
    <shopcart v-if="reFresh" ref="shopcart" :selectFoods="selectFoods" :paymentList="paymentList" @totalPrice="totalPrice"
      @shopCarSingleFood="shopCarSingleFood" @shopCarDecreaseFood="shopCarDecreaseFood" @shopCarDecreaseNMFood="shopCarDecreaseNMFood"
      @emptyShopCar="emptyShopCar"></shopcart>
    <pay-way v-if="paymentList.length > 1" :payPrice="payPrice" :paymentList="paymentList" :selectFoods="selectFoods"
      @emptyShopCar="emptyShopCar"></pay-way>
    <!-- 公告 -->
    <meal-notice :showFold="$store.state.fold" :mealNoticeListData="mealNoticeListData" :mealNoticeData="mealNoticeData"
      @hideState="hideNotice"></meal-notice>
    <!-- 选择疾病 -->
    <choose-diseases></choose-diseases>
    <food @add="addFood" @decrease="decreaseFood" :detailFood="detailFood" :foodInfo="foodInfo" :NMItemPrams="NMItemPrams"
      ref="food"></food>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...
      selectFoods() {
        ...
        if (this.currentMenuList.orderingMode == 1) {
          ...
          return foods
          // debugger
          // if (!this.currentMenuList) return foods

          // this.currentMenuList.dataList[0].repastDataList.forEach(good => {
          //     good.menuList.forEach(food => {
          //         if (food.count) {
          //             foods.push(food)
          //         }
          //     })
    ...
    methods: {
        ...
        _this.$ajax
          ...
          .then(res => {
            ...
            if (res.data.code == '0') {
              ...
              //成功
              let parentId =
                _this.currentModuleId + '_' + _this.currentRepastId
              ...
              if (!result) {
                ...
                if (res.data.data.orderingMode === 1) {
                  for (let i = 0; i < tempMenu.dataList[0].repastDataList.length; i++) {
                    for (let j = 0; j < tempMenu.dataList[0].repastDataList[i].menuList.length; j++) {
                      tempMenu.dataList[0].repastDataList[i].menuList[j].moduleId = _this.currentModuleId
                      tempMenu.dataList[0].repastDataList[i].menuList[j].repastId = _this.currentRepastId
                      tempMenu.dataList[0].repastDataList[i].menuList[j].moduleName = _this.currentModuleName
                      tempMenu.dataList[0].repastDataList[i].menuList[j].repastName = _this.currentRepastName
                      tempMenu.dataList[0].repastDataList[i].menuList[j].orderingMode = res.data.data.orderingMode
                      tempMenu.dataList[0].repastDataList[i].menuList[j].menuTypeName = tempMenu.dataList[0].repastDataList[
                        i].menuTypeName
                      tempMenu.dataList[0].repastDataList[i].menuList[j].count = 0
                      //提交订单使用
                      tempMenu.dataList[0].repastDataList[i].menuList[j].mealDate = res.data.data.mealDate
                      tempMenu.dataList[0].repastDataList[i].menuList[j].cateringTypeId = res.data.data.cateringTypeId
                      tempMenu.dataList[0].repastDataList[i].menuList[j].cateringId = res.data.data.cateringId
                      tempMenu.dataList[0].repastDataList[i].menuList[j].cateringTime = res.data.data.cateringTime
                      tempMenu.dataList[0].repastDataList[i].menuList[j].dietAdviceId = res.data.data.dietAdviceId

                      //遍历购物车缓存
                      let myCurrentCarFoods = myCarFoods.filter(n =>
                        n.orderingMode == 1 &&
                        n.moduleId == _this.currentModuleId &&
                        n.repastId == _this.currentRepastId)
                      var foodIndex = myCurrentCarFoods.findIndex(n => n.code == tempMenu.dataList[0].repastDataList[
                        i].menuList[j].code)
                      if (foodIndex != -1)
                        tempMenu.dataList[0].repastDataList[i].menuList[j].count = myCurrentCarFoods[foodIndex].count
                    }
                  }
                } else if (res.data.data.orderingMode === 2) {
                  for (let i = 0; i < tempMenu.dataList[0].menuList.length; i++) {
                    tempMenu.dataList[0].menuList[i].moduleId = _this.currentModuleId
                    tempMenu.dataList[0].menuList[i].repastId = _this.currentRepastId
                    tempMenu.dataList[0].menuList[i].orderingMode = res.data.data.orderingMode
                    tempMenu.dataList[0].menuList[i].count = 0
                    //提交订单使用
                    tempMenu.dataList[0].menuList[i].mealDate = res.data.data.mealDate
                    tempMenu.dataList[0].menuList[i].cateringTypeId = res.data.data.cateringTypeId
                    tempMenu.dataList[0].menuList[i].cateringId = res.data.data.cateringId
                    tempMenu.dataList[0].menuList[i].cateringTime = res.data.data.cateringTime
                    tempMenu.dataList[0].menuList[i].dietAdviceId = res.data.data.dietAdviceId
                    ...
                    let myCurrentCarFoods = myCarFoods.filter(n =>
                      ...
                      n.moduleId == _this.currentModuleId &&
                      n.repastId == _this.currentRepastId)
                    var foodIndex = myCurrentCarFoods.findIndex(n => n.code == tempMenu.dataList[0].menuList[i].code)
                    if (foodIndex != -1)
                      tempMenu.dataList[0].menuList[i].count = myCurrentCarFoods[foodIndex].count
                  }
                ...
                if (res.data.data.orderingMode === 1 && _this.currentModuleId != _this.oldCurrentModuleId) {
                  // _this.dandian=false
                  // _this.dandian=true
                  _this.$nextTick(() => {
                    _this._initScroll()
      ...
      //菜肴详情
      selectFood(food, event, outIndex, mealItem, price, itemN, itemM, itemMenuTypeName) {
        if (!event._constructed) {
          return
        }
      ...
      updateMenuList(food) {
        ...
        if (food.orderingMode == 1) {
          // debugger
          let menuIndex = this.menuList.findIndex(n =>
            n.parentId == (food.moduleId + '_' + food.repastId))
          if (menuIndex != -1) { //找到了早中晚
            this.menuList[menuIndex].dataList[0].repastDataList.forEach(n => {
              let foodIndex = n.menuList.findIndex(n => n.code == food.code)
              if (foodIndex != -1)
                n.menuList[foodIndex].count = food.count
              return
            })
          ...
          let menuIndex = this.menuList.findIndex(n =>
            n.parentId == (food.moduleId + '_' + food.repastId))
          if (menuIndex != -1) { //找到了早中晚
            let foodIndex = this.menuList[menuIndex].dataList[0].menuList.findIndex(n => n.code == food.code)
            if (foodIndex != -1)
              this.menuList[menuIndex].dataList[0].menuList[foodIndex].count = food.count
          }
        } else if (food.orderingMode == 3) {
          let menuIndex = this.menuList.findIndex(n =>
            n.parentId == (food.moduleId + '_' + food.repastId))
          if (menuIndex != -1) { //找到了早中晚
            let foodIndex = this.menuList[menuIndex].dataList[0].repastDataList[0].menuList.findIndex(n => n.code ==
              food.code)
            if (foodIndex != -1)
              this.menuList[menuIndex].dataList[0].repastDataList[0].menuList[foodIndex].count = food.count
          }
        }
      },
      //购物车中单点加减号回调
      ...
      shopCarDecreaseFood(food) {
        ...
        if (menuIndex != -1) {
          ...
          let foodIndex = menulistTemp.findIndex(n =>
            n.code == food.code);
          if (foodIndex != -1)
            menulistTemp[foodIndex].count = 0
        }
        if (this.$route.query.id == 1) {
          //今日加餐
          storage.setObjItem('mycar1', this.selectSetFoods)
      ...
      shopCarDecreaseNMFood(food) {
        ...
        if (menuIndex != -1) {
          ...
          // let menulistTemp1= this.menuList[0].dataList[0].repastDataList[0].menuList
          // debugger
          let menulistTemp = this.menuList[menuIndex].dataList[0].repastDataList[food.NMIndex].menuList
          let foodIndex = menulistTemp.findIndex(n =>
            n.code == food.code);
          if (foodIndex != -1)
            menulistTemp[foodIndex].count = 0
        ...
        if (item.count === 1) { //取消选中
          ...
          if (filterGoods.length > 0 && filterGoods.length >= filterGoods[0].M) {
            if (filterGoods[0].M == 1) {
              ...
              // debugger
              this.currentMenuList.dataList[0].repastDataList[filterGoods[0].NMIndex].menuList.forEach(function(value,
                index, array) {
                value.count = 0
              });
              //this.currentMenuList
      ...
      choosedSetMeal(item) {
        ...
        if (item.count === 1) { //取消选中
          ...
          this.currentMenuList.dataList[0].menuList.forEach(function(value, index, array) {
            value.count = 0
          });
          item.count = 1;
          //套餐
        ...
        .food-item {
          ...
          .foodNutrient {
            ...
            &::after {
              ...
              top: -10px;
              left: 36px;
              border-color: transparent transparent rgba(219, 219, 234, 0.4) transparent;
            }
