  ...
  <div :class="['recommend', flag ? 'bigSize' : '']">
    ...
    <div class="loginModel" v-show="isLogin">
      <div class="alreadyLogin">
        <!-- 不走充值结算模式，his计费等 -->
        <div class="Unbound" @click="UnboundInfo"  v-show="isLoginBindDelete">解绑信息</div>
        <div class="main">
          <div class="media">
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      storage.setItem('userStatus', true);//判断用户是否登陆状态、此处固定成已登录状态

      /**
       * 将url中携带的参数当做唯一标识符，取到唯一标识符后当成openid做所有操作
       */
      if (this.$route.query.patCode == ''||this.$route.query.patCode == null||this.$route.query.patCode == undefined) {
        console.log('链接中未发现参数，暂不做处理')
      } else{
        storage.setItem('openId', 'des_'+this.$route.query.patCode);//若存在参数，则缓存为openid(暂时写成固定，方便测试)
      }
      if (storage.getItem('userStatus')) {
        this.isLogin = true
        this.$store.commit('changeIsLogin', true)
      ...
      GetPatientInfo() {
        ...
        _this.$ajax
          ...
          .then(resp => {
            ...
            console.log(resp)
            if (resp.data.code == 0) {
              // resp.data.data.isLeaveHospital=1
