  ...
  <div :class="['recommend', flag ? 'bigSize' : '']">
    ...
    <div class="cube-slide-group">
      <div v-show="recommends && recommends.length == 0">
        <img src="../assets/images/bannerQ.png" @click="bannerQuestion" />
      </div>
    ...
    <div class="loginModel" v-show="isLogin">
      <div class="alreadyLogin">
        ...
        <div class="main">
          ...
          <div class="tagCon">
            <span class="tag"><span class="tagTxt">{{
                                info.mealName
                            }}</span></span>
            <span v-show="info.nutritionEducation" class="more" @click="goArticle(info.nutritionEducation)">
        ...
        <div class="account">
          <!-- 医院不走线上充值模式，仅能看交易明细 -->
          <div @click="tradeDetails()" class="account1" v-show="enableRecharge == '0'">
            交易明细
          </div>
          <!-- 显示余额 -->
          <!-- 0：未结算，1：已结算 -->
          ...
          <div class="account2" v-show="enableRecharge == '1'">
            ...
            <template v-if="isSettlement == 0">
              ...
              <div class="accountR">
                ...
                <template v-if="info.isLeaveHospital === 0">
                  <button v-if="info.balance > warningValue" class="accountBtn" @click="goRecharge(info.balance)">
                    账户中心
                  </button>
                  <button v-else class="accountBtn" @click="goRecharge(info.balance)">
                    去充值
                  </button>
                </template>
                <template v-else>
                  <button class="accountBtn" @click="tradeDetails()">
                    交易明细
                  </button>
                </template>
              </div>
            ...
            <template v-else>
              ...
              <div class="accountR">
                <template v-if="info.isLeaveHospital === 0">
                  <button class="accountBtn" @click="goRecharge(info.balance)">
                    去充值
                  </button>
                </template>
                <template v-else>
    ...
    <div class="menuList" :class="menuListClass">
      <ul>
        <li class="item" :class="itemClass" v-for="(item, index) in menuList" :key="index">
          <template v-if="item.name == '明日订餐'">
            <router-link :to="{ path: 'orderFood', query: { id: item.id } }">
              <img class="img" :src="item.picUrl" />
              <div class="txt">{{ item.name }}</div>
            </router-link>
          </template>
          <template v-else-if="item.name == '今日加餐'">
            <router-link :to="{ path: 'orderFood', query: { id: item.id } }">
              <img class="img" :src="item.picUrl" />
              <div class="txt">{{ item.name }}</div>
            </router-link>
    ...
    <div class="articleNavBar">
      ...
      <sticky-slot class="stickyTop">
        <div class="articleScrollNav">
          <cube-scroll ref="scroll" :data="articlesType" direction="horizontal" class="horizontal-scroll-list-wrap">
            <ul class="list-wrapper">
              <li v-for="(item, index) in articlesType" :key="index" class="list-item" :class="{ active: index === nowIndex }"
...
<script>
  ...
  export default {
    ...
    created() {
      console.log('----是否登陆状态------判断的----'+storage.getItem('userStatus'))
      // console.log(currentAppId)
      // alert(currentAppId)

      // 根据url链接中的参数获取openid并缓存
      // var url_openid = this.$route.query.openId;
      // if (url_openid==null||url_openid==undefined||url_openid=='') {
      //   console.log('不存在openid，不做任何操作')
      // } else{
      //   console.log('存在openid')
      //   storage.setItem('openId', url_openid)
      // }

      //普通进入页面方式和支付回调进入页面方式
      if (this.$route.query.orderid == '' || this.$route.query.orderid == null || this.$route.query.orderid ==
        undefined) {
        //正常进入
      } else if (this.$route.query.orderid == 'chongzhiok') {
        // chongzhisuccess

        var merchantpayokgwc = storage.getItem('merchantTradeIdpayok')
        if (merchantpayokgwc == this.$route.query.merchantTradeId) {
          return;
        }
        //CryptoJS
        var data = new Date();
        ...
        var currrentTime = year + '-' + month + '-' + day + ' ' + hour + ':' + minutes + ':' + seconds;
        var currrentTimeL = JSON.stringify(year) + month + day + hour + minutes + seconds;
        let key = 'YkBrWXCz';
        let message = 'wincome' + '+' + currrentTimeL + '+' + this.$route.query.price;
        ...
        var jiami = encrypted.ciphertext.toString();
        var amountEncrypt = jiami.toUpperCase()

        console.log('走了充值回调路径')
        var _this = this
        let url = this.api.userApi.Recharge
        let weixinNo = storage.getItem('openId')
        let price = this.$route.query.price
        console.log(weixinNo)
        console.log(price)
        let authCode = encryptionRechange(weixinNo, 107, price)
        let currentDate = formatDate(new Date(), 'yyyyMMddhhmmss')
        let callTime = formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss')
        // var amountEncrypt = this.$route.query.amountEncrypt
        var merchantTradeId = this.$route.query.merchantTradeId
        let parmas = {
          authCode: authCode,
          terminalNo: weixinNo,
          callFrom: 'wincome',
        ...
        _this.$ajax
          ...
          .then(res => {
            console.log(res)
            storage.setItem('merchantTradeIdpayok', merchantTradeId)
        ...
        // payok
        var _this = this
        let url = this.api.userApi.OrderPaid
        let weixinNo = storage.getItem('openId')
        let price = this.$route.query.price
        let authCode = encryption(weixinNo, 112)
        let currentDate = formatDate(new Date(), 'yyyyMMddhhmmss')
        let callTime = formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss')
        // var amountEncrypt = this.$route.query.amountEncrypt
        var merchantTradeId = this.$route.query.merchantTradeId
      ...
      this.GetModuleList()
      this.GetIllness()
      // this.GetActicleList() //文章列表
      this.openid = storage.getItem('openId')
      // this.openid = 'ocWCVwkas3DM6IQhJrpwTonAbTFc'
    ...
    methods: {
      bannerQuestion() {
        window.location.href = 'https://www.wenjuan.com/s/iiuEre/'
      },
      goBack() {
